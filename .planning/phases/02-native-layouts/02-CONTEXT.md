...
- **Estimated +700-1000 LOC** of net GDScript over the Phase 1 baseline (~530 LOC); cumulative ~1230-1530 LOC. End-of-Phase identity-guardrail audit per ROADMAP.md. Detailed per-file breakdown in `<code_context>` § LOC budget below.
...
- **D-68: The 7-wave breakdown from the FOURTH SUPERSESSION (D-67) is locked verbatim; not re-litigated this round.** The size of Phase 2 (~30 reqs, 17 success criteria) was discussed and the user chose to keep it as one phase rather than split into Phase 2.0 / 2.5 — the dependency graph is tight enough that a split would not buy clarity.
  ...
  - **Wave 2 — Synthesis machinery + overlay deletion + contract deletion + demo rebind.** Build `_synthesize_strip(strip_index, mode)` covering all 5 modes (ONE/TWO/THREE/FOUR/FIVE); fill the `get_fallback_tile_set()` stub from Wave 1 with runtime TileSet construction + collision/occlusion/navigation polygon copy. Delete `_overlay_layer`, `_OVERLAY_LAYER_NAME`, `_paint_overlay_for_slot()`, `AtlasSlot.diagonal_complement_atlas_coords`. Delete `PentaTileAtlasContract` class + `addons/penta_tile/contracts/` folder + `penta_tile_atlas_contract.gd` + `addons/penta_tile/penta_tile_template.png`. **`_DEFAULT_LAYOUT` cleanup is a single atomic task** — the static singleton declaration at `penta_tile_map_layer.gd:54-58` (comment block + `static var _DEFAULT_LAYOUT`) AND its consumer inside `_resolve_layout` at lines 196-198 (the `if _DEFAULT_LAYOUT == null: ... return _DEFAULT_LAYOUT` block) MUST land in the same commit. Splitting them leaves an intermediate broken state where the singleton is gone but `_resolve_layout` still references it, or vice versa. Replace `atlas_contract` property with `layout: PentaTileLayout` (idempotence guard + disconnect-before-reconnect on `layout.changed`); rewrite `_resolve_layout` to read `self.layout` directly. **Atomically rebind `addons/penta_tile/demo/penta_tile_demo.tscn`** — non-skippable Wave 2 acceptance criterion (LAYER-04).
  ...
  - **Wave 5 — Asset relocation + README path retarget.** Delete `addons/penta_tile/templates/` folder. Migrate the 5 existing flat PNGs to co-located paths: 10 in `addons/penta_tile/layouts/penta_tile_layout_penta/{one,two,three,four,five}_{horizontal,vertical}.png`; 4 flat siblings (`penta_tile_layout_dual_grid_16.png`, `penta_tile_layout_wang_2_edge.png`, `penta_tile_layout_wang_2_corner.png`, `penta_tile_layout_minimal_3x3.png`). Update / rename the bitmask generator script (renamed from `_generate_greybox_templates.py`). **README image-src retarget is non-skippable** (atomic with the PNG migration in this same wave): three `<img>` tags in `README.md` (currently lines 5, 30, 70) reference `addons/penta_tile/templates/penta_horizontal.png`. The deletion of `templates/` makes them dead links mid-Phase-2. The canonical "What is a Penta tileset?" diagram on line 30 is **load-bearing for the codename per `CLAUDE.md` § Coined-Term Discipline** — it cannot be allowed to 404 even briefly. Plan-phase picks the retarget destination (suggest `addons/penta_tile/layouts/penta_tile_layout_penta/five_horizontal.png` for line 30 since the FIVE-mode template visually shows all five explicit archetypes matching the alt-text; lines 5 and 70 follow). Wave 5 commit must include the README edit in the same git commit as the PNG migration so no intermediate state has broken images.
...
- `addons/penta_tile/penta_tile_map_layer.gd` (298 LOC) — the layer file. Line citations verified against the file at the time of writing; widen ranges if downstream agents see drift. Deletes in Wave 2: `_OVERLAY_LAYER_NAME` constant (line 7), `_overlay_layer` field (line 46), `_DEFAULT_LAYOUT` static singleton (declaration at lines 54-58 — comment block 54-57 + `static var` at 58; consumer in `_resolve_layout` at lines 196-198 — the `if _DEFAULT_LAYOUT == null: ... return _DEFAULT_LAYOUT` block), `_paint_overlay_for_slot` (lines 177-181). Renames in Wave 2: `atlas_contract` property (line 14) → `layout`. Modifies in Wave 2: `_ensure_visual_layers` (212-217 — drop overlay branch), `_paint_via_layout` (146-158 — drop overlay paint call), `_clear_visual_layers` (266-269 — drop overlay layer iteration), `_sync_visual_layers` (234-250 — drop overlay layer iteration), `_resolve_layout` (193-198 — rewrite to read `self.layout` directly; this rewrite IS the singleton-consumer deletion, so the two land in the same edit), `_on_contract_changed` (297-298 — rename to `_on_layout_changed`).
...
- `addons/penta_tile/layouts/penta_tile_layout.gd` — modified in Wave 1: rename `template_image` → `bitmask_template` (LAYOUT-03); remove `fallback_tile_set` @export (LAYOUT-03); add `get_fallback_tile_set()` virtual stub (LAYOUT-06); delete `decoder_image`. Filled in Wave 2 with the default `get_fallback_tile_set()` implementation that builds a TileSet from `bitmask_template`.
...
- **`PentaTileLayoutPentaHorizontal.mask_to_atlas`** (16-state match) — the case-table is being relocated into `PentaTileLayoutPenta` with the new slot ordering (slot 0 = IsolatedCell, slot 1 = Fill, slot 2 = Border, slot 3 = InnerCorner, slot 4 = OppositeCorners). The 16 cases each remap to the new slot indices. **How OuterCorner cases resolve is GATED BY D-69** — the assertion in earlier drafts that "OuterCorner cases resolve to slot 0 with rotation transforms" front-runs the still-undefined sub-region anchoring spec. Plan-phase locks the actual mapping (which slot, which transform, which sub-region origin) as part of the D-69 deliverable; do not bake an anchoring assumption into PLAN.md tasks before that gate is resolved.
...
- **Stale README content (Phase 5 DOC-01..03 scope, NOT touched in Phase 2 except for the Wave 5 path retarget).** Audited at HEAD; the following sections directly contradict the Phase 2 architecture and need a Phase 5 rewrite — deliberately out-of-scope here so Phase 2 stays atomic on the architectural sweep:
  - Lines 24, 25, 91, 103: describe Penta as a "4-tile" minimum (stale; the FIVE mode is the new ceiling, ONE is the new floor).
  - Line 30 IMG alt-text describes the 5-archetype Penta but currently points to the 4-tile v0.1 atlas — Wave 5 retargets the path; the alt-text/image semantic alignment is finalized when Phase 5 ships the new 5-archetype canonical PNG.
  - Lines 68-79 "🎨 The Penta-System Template" section: still describes 4 archetypes (Fill / Inner Corner / Border / Outer Corner) and "internal overlay layer" diagonal compositing. Directly contradicts the canonical "What is a Penta tileset?" section above it (which describes 5 archetypes + slot 0 IsolatedCell + synthesis). Phase 5 either deletes this section or rewrites against the new architecture.
  - Line 91 "two-layer composition for diagonals" in the TileMapDual comparison table: stale (overlay layer deleted in Phase 2).
  - Lines 114-124 "Addon Layout" tree: shows the v0.1 file structure (no `contracts/`, no `layouts/`, includes the now-deleted `penta_tile_template.png`).
  - Line 145 `atlas_layout` row in the Current API table: that property was deleted in Phase 1 (replaced by `atlas_contract`, now `layout` after Phase 2).
  - Lines 172-181 "Implementation Notes": describes the diagonal masks 6/9 being drawn via the overlay layer. Phase 2 deletes that path entirely.
  - Lines 189-197 "Roadmap" backlog: PentaBake (TOOL-01) framing as "compose a fifth edge/diagonal connector tile" is partially obviated by Phase 2's synthesis machinery; Phase 5 should reframe the backlog to match the v2 backlog in REQUIREMENTS.md.
